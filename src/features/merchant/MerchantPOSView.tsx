  ...
  useEffect(() => {
    ...
    OfflineSyncService.receiveReplicatedTransactions(async (data: any) => {
      if (data.type === 'transaction' && data.data.receiverWallet === merchantWallet.did) {
        ...
        console.log('[Merchant] Listener received data.type:', data.type, 'payload:', data);

        if (data.type === 'payment_handshake') {
          const handshake = data.data as any; // Type is PaymentSessionHandshake
          console.log('[Merchant] Checking handshake merchantDid:', handshake.merchantDid, 'against merchantWallet.did:', merchantWallet.did);
          
          if (handshake.merchantDid === merchantWallet.did) {
            console.log('[Merchant] Handshake merchantDid MATCHED! Checking active request');
            const activeReq = currentRequestRef.current;
            console.log('[Merchant] currentRequestRef.current:', activeReq);

            if (!activeReq) {
              console.log('[Merchant] Condition FAILED: currentRequestRef.current is empty (no active payment generated by merchant)');
            } else {
              console.log('[Merchant] Comparing activeRequest.sessionId:', activeReq.sessionId, 'with handshake.sessionId:', handshake.sessionId);
              if (activeReq.sessionId === handshake.sessionId) {
                console.log('[Merchant] Session ID MATCHED! Creating targeted payment request');
                setStatus('handshake_received' as any);

                // Add target DID
                const targetedReq = { ...activeReq, targetDid: handshake.customerDid };
                console.log('[Merchant] PaymentRequest created and sent:', targetedReq);
                OfflineSyncService.sendPaymentRequest(targetedReq).catch(console.error);
              } else {
                console.log('[Merchant] Condition FAILED: activeRequest.sessionId does not match handshake.sessionId');
              }
            }
          } else {
            console.log('[Merchant] Condition FAILED: handshake.merchantDid does not match merchantWallet.did');
          }
        }
      }
    });
